...
本文档提供了理解和复现vLLM的分步指南。遵循此路径以获得最佳学习体验。
...
首先构建基本神经网络的`layers`。这些包含模型构建的基础块。

### 1.1 激活函数 ✅

从构建激活函数开始 (例如 SiLU, GELU).

**关键知识: 使用 `torch.compile` 进行优化**
- 进行基准测试:
	```python
...
**Takeaway:** `torch.compile` 有助于进行更大的计算，但为较小的操作增加了开销（算子进行编译初始化的耗时 > 算子计算本身耗时）。
